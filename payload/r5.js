...
/*   getAll: (req, res) => {
  ...
  getAddon: (req, res) => {
    let query = 
    `SELECT * FROM r5 WHERE  ${req.params.key} 
    LIKE "%${req.params.value}%" AND addon_status = ${req.params.status}`;
  ...
  /* deleteAddon: (req, res) => {
    ...
    `DELETE FROM r5 WHERE ${req.params.key} = "${req.params.value}"` :
    `DELETE FROM r5 WHERE ${req.params.key} = ${req.params.value}`;
